...
static void test_framerate_visual(CFBD_OLED* handle)
{
    ...
    const uint8_t dot_size = 15; // 光点大小
...
CFBD_OLED ssd1327;
void test_scan_rows(CFBD_OLED* oled);
void do1327Demo()
{
    ...
    params.device_specifics = getSSD1327Specific();
    params.accepted_time_delay = -1; // never time outs
    params.device_address = SSD1327_DRIVER_ADDRESS;
    params.i2cHandle = &handle;
    params.iic_transition_callback = NULL;

    CFBD_GetOLEDHandle(&ssd1327, CFBD_OLEDDriverType_IIC, &params, CFBD_TRUE);

    test_set_pixel_line(&ssd1327, 4, 3);
    system_delay_ms(1000);
    ssd1327.ops->clear(&ssd1327);
    ssd1327.ops->update(&ssd1327);

    CFBD_GraphicDevice graphic_device;
    CFBDGraphic_BindDevice(&graphic_device, OLED, &ssd1327);
    CFBDGraphic_DeviceSetIfRequestUpdateAtOnce(&graphic_device, CFBD_TRUE);

    test_draw_base_graphic(&graphic_device);

    system_delay_ms(1000);
    uint8_t grey_scale = 0x0F;
    graphic_device.ops->self_sets(&graphic_device, "color", NULL, &grey_scale);

    test_framerate_visual(&ssd1327);
